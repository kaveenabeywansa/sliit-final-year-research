    ...
    while True:
        ...
        if N > 250 :                                   # avoid high frequency noise
            if  (Signal > thresh) and  (Pulse == False) and  (N > (IBI/5.0)*3.0)  :       
              ...
              print 'HRV: {}'.format(IBI)
              file1 = open("bpm.txt","w")
              file1.write(str(BPM))
              file1.close()
              
              hrvArray.append(IBI)
              if len(hrvArray) > 20:
                  stk = np.hstack(hrvArray)
                  mean, std = np.mean(stk), np.std(stk)
                  # print("Mean: ",mean)
                  # print("Std: ",std)
                  # print(hrvArray)
                  hrvArray.pop(0)
                  stressLevel=(std/mean)*100
                  print("Stress Rate: ", stressLevel)
                  files = open("stress.txt","w")
                  files.write(str(stressLevel))
                  files.close()
                  if (std/mean)*100 > 51:
                    print("You are stressed")
                  else:
                    print("You are Relaxed")
